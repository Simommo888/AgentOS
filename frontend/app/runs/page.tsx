...
import type { AgentLog, AgentRun, RunQueue as RunQueueType } from "@/lib/types";
...
export default function RunsPage() {
  ...
  async function loadRuns() {
    const [runData, queueData] = await Promise.all([api.runs({ limit: 100 }), api.runQueue()]);
    setRuns(runData);
    setQueue(queueData);
    if (!selectedId && runData.length > 0) {
  ...
  async function refresh() {
    await loadRuns();
    if (selectedId) {
      setLogs(await api.runLogs(selectedId));
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">查看 Agent 运行队列、实时状态、耗时、错误和输出路径。</p>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          <RunLogViewer logs={logs} isRunning={selectedIsRunning} />
        </CardContent>
      </Card>
    </div>
  );
